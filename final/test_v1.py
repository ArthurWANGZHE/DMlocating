...
class HikCamera:
    ...
    def Connect(self):
        ...
        for i in range(0, deviceList.nDeviceNum):
            ...
            if mvcc_dev_info.nTLayerType == MV_GIGE_DEVICE:
                ...
                print(f'self.SerialIndex: {self.SerialIndex}')
                if tempStr == self.SerialIndex:
                    nConnectionNum = i
                    print(f'i: {i}')

        if nConnectionNum < 0:
            print("Search camera(" + self.SerialIndex + ") failed!")
            return
        ...
        if ret != 0:
            print("set software trigger mode fail! ret[0x%x]" % ret)
            return
    ...
    def DisConnect(self):
        ...
        if ret != 0:
            print("stop grabbing fail! ret: %s" % ret)
            return
