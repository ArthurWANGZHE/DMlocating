...
class ConvertManager (object):
    ...
    def load_labels (cls,name_file):
        """
        load_names from file.one name one line 
        :param name file :
        :return :
        """
    ...
    def _rectangle_points_to_polygon (self, points ):
        xmin =0
        ymin =0
        xmax =0
        ymax =0
    ...
    def convert_dataset(self,json_dir , json_list , images_dir , labels_dir , names , save_mode =' train '):
         ...
         for file in tqdm ( json_list ):
            ...
            for shape in data_dict ['shapes']:
                ...
                if shape [' shape _ type ']==' rectangle ':
                    points = self._rectangle_points_to_polygon(shape ['points'])
                    for point in points :
    ...
    def split_train_val_test_dataset ( self , file_list , train_ratio =0.9, trainval_ratio =0.9, need_test_dataset = False ,shuffle_list = True ):
        if shuffle_list :
...
if __name__ ==' __main__ ':
    cm = ConvertManager()
    cm.start(r'/home/robot/'
             r'/dataset/labelme-dogcat-test', r'/home/robot/ultralytics-main/dataset/mydata ')
